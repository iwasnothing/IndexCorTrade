...
class Hypertune(object):
    ...
    def __call__(self, trial):
        config = {
            "seq_len": trial.suggest_int("seq_len", 5, 10),
            "hidden_size": trial.suggest_int("hidden_size", 10, 100),
            "batch_size": trial.suggest_int("batch_size", 30, 60),
...
def mainTest(num_samples, num_epochs, sym01, sym02, period):
    ...
    study.optimize(Hypertune(num_samples, num_epochs, sym01, sym02, period), n_trials=10)
    ...
    print(best_config)
    model, loss = myTrain(best_config, num_epochs * 10, sym01, sym02, period)
    model.eval()
    model.freeze()
