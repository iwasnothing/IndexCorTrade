...
class MyDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        y0 = date.today().strftime('%Y-%m-%d')
        y1 = ( date.today() - timedelta(days=365) ).strftime('%Y-%m-%d')
        y2 = ( date.today() - timedelta(days=2*365) ).strftime('%Y-%m-%d')
        y3 = ( date.today() - timedelta(days=3*365) ).strftime('%Y-%m-%d')
        d1 = getFX(y1,y0)
        d2 = getFX(y2,y1)
        d3 = getFX(y3,y2)
        df1 = pd.concat([d1,d2,d3], ignore_index=True)
        currencies = [x for x in df1.columns if "date" not in x]
        df = df1.merge(hist1, left_on='ondate', right_on='Date').reset_index()
        df = df.sort_values("ondate",ascending=True)
        df['year'] = df['ondate'].apply(lambda x: int(x.year))
        df['month'] = df['ondate'].apply(lambda x: int(x.month))
        df['day'] = df['ondate'].apply(lambda x: int(x.day))
        #df['log-vol'] = df['volume'].apply(lambda x: math.log(1 + x))
        tgt = 'close-y-next-diff'
        df[tgt] = df['Close'].shift(-1)
        df[tgt] = df[[tgt, 'Close']].apply(lambda x: (x[tgt] - x['Close'])*100/x['Close'], axis=1)
        print(df.columns)
        lag=10
        for f in currencies:
        ...
        print("last date is",df['ondate'].iloc[-1])
        ...
        features = ['PCA'+str(a) for a in range(1,1+pcacnt)]
        pf = pd.DataFrame(X_reduced, columns=features)
        print("pca len",len(pf))
        for k in range(1,pcacnt+1):
            c1 = "PCA" + str(k)
            df[c1] = pf[c1]
...
class Hypertune(object):
    def __init__(self, num_samples, num_epochs, sym01, sym02, period):
    ...
    def __call__(self, trial):
        config = {
            ...
            "batch_size": trial.suggest_int("batch_size", 20, 30),
            "dropout": trial.suggest_uniform("dropout", 0.1, 0.2),
            "lr": trial.suggest_uniform("lr", 1e-5, 1e-2),
            "num_layers": trial.suggest_int("num_layers", 2, 4)
        }
        model, loss = myTrain(config, self.num_epochs, self.sym01, self.sym02, self.period)
        # (score,acc,y_tmr) = myTest(model,config)
...
def myTest(model,best_config,sym01, sym02, period):
...
def mainTest(num_samples, num_epochs, sym01, sym02, period):
    ...
    study.optimize(Hypertune(num_samples, num_epochs, sym01, sym02, period), n_trials=20)
    ...
    print(best_config)
    model, loss = myTrain(best_config, num_epochs * 5, sym01, sym02, period)
    print(model)
    model.eval()
    model.freeze()
    (score, acc, y_tmr,baseline) = myTest(model, best_config,sym01, sym02, period)
...
def getFX(fromDate,toDate):
    #url_query = "https://api.exchangeratesapi.io/history?start_at="+fromDate+"&end_at="+toDate+"&symbols=EUR,JPY,AUD,CAD&base=USD"
    url_query = "https://api.exchangeratesapi.io/history?start_at="+fromDate+"&end_at="+toDate+"&base=USD"
    print(url_query)
    raw_response = requests.get(url_query)
    #print(raw_response)
    rsp = raw_response.json()
    rates = rsp["rates"]
    #print(rates)
    list = []
    ...
    df1['ondate'] = df1['date'].apply(lambda x: datetime.strptime(x,"%Y-%m-%d"))
    return df1
#
#
#
...
def compareAll(mkt):
    ...
    if len(good) > 0 :
        ans = sum(good[['score','tmr']].apply(lambda x: x['score']*x['tmr'],axis=1))/sum(good['score'])
        print('ans',ans)
        return ans
    else:
        return 0
